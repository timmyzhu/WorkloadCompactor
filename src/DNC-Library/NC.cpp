...
FlowId NC::initFlow(Flow* f, const Json::Value& flowInfo, ClientId clientId)
{
    ...
    f->latency = 0;
    f->ignoreLatency = c->ignoreLatency;
    return flowId;
}
...
ClientId NC::initClient(Client* c, const Json::Value& clientInfo)
{
    ...
    c->latency = 0;
    bool closedLoop = clientInfo.isMember("closedLoop") ? clientInfo["closedLoop"].asBool() : false;
    bool bestEffort = clientInfo.isMember("bestEffort") ? clientInfo["bestEffort"].asBool() : false;
    c->ignoreLatency = closedLoop || bestEffort;
    return clientId;
}
